...
void focus_view(struct dgde_view *view, struct wlr_surface *surface) {
  ...
  struct dgde_server *server = view->server;
...
static bool view_at(struct dgde_view *view, double lx, double ly,
...
void begin_interactive(struct dgde_view *view, enum dgde_cursor_mode mode,
  ...
  if (mode == DgdeCursor_Move) {
    ...
    server->grab_x = pos.x - view->x;
    server->grab_y = pos.y - view->y;
  } else {
